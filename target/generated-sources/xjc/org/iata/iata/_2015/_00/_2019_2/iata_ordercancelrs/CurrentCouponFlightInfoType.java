//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.3.0 
// See <a href="https://javaee.github.io/jaxb-v2/">https://javaee.github.io/jaxb-v2/</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2023.11.14 at 08:38:47 PM CST 
//


package org.iata.iata._2015._00._2019_2.iata_ordercancelrs;

import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlType;


/**
 * Latest flight information as it relates to a coupon (e.g. Current, CheckedIn, Flown).
 * 
 * <p>Java class for CurrentCouponFlightInfoType complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType name="CurrentCouponFlightInfoType"&gt;
 *   &lt;complexContent&gt;
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType"&gt;
 *       &lt;choice&gt;
 *         &lt;element name="CheckedInAirlineInfo" type="{http://www.iata.org/IATA/2015/00/2019.2/IATA_OrderCancelRS}PaxSegmentType"/&gt;
 *         &lt;element name="CurrentAirlineInfo" type="{http://www.iata.org/IATA/2015/00/2019.2/IATA_OrderCancelRS}PaxSegmentType"/&gt;
 *         &lt;element name="FlownAirlineInfo" type="{http://www.iata.org/IATA/2015/00/2019.2/IATA_OrderCancelRS}PaxSegmentType"/&gt;
 *       &lt;/choice&gt;
 *     &lt;/restriction&gt;
 *   &lt;/complexContent&gt;
 * &lt;/complexType&gt;
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "CurrentCouponFlightInfoType", propOrder = {
    "checkedInAirlineInfo",
    "currentAirlineInfo",
    "flownAirlineInfo"
})
public class CurrentCouponFlightInfoType {

    @XmlElement(name = "CheckedInAirlineInfo")
    protected PaxSegmentType checkedInAirlineInfo;
    @XmlElement(name = "CurrentAirlineInfo")
    protected PaxSegmentType currentAirlineInfo;
    @XmlElement(name = "FlownAirlineInfo")
    protected PaxSegmentType flownAirlineInfo;

    /**
     * Gets the value of the checkedInAirlineInfo property.
     * 
     * @return
     *     possible object is
     *     {@link PaxSegmentType }
     *     
     */
    public PaxSegmentType getCheckedInAirlineInfo() {
        return checkedInAirlineInfo;
    }

    /**
     * Sets the value of the checkedInAirlineInfo property.
     * 
     * @param value
     *     allowed object is
     *     {@link PaxSegmentType }
     *     
     */
    public void setCheckedInAirlineInfo(PaxSegmentType value) {
        this.checkedInAirlineInfo = value;
    }

    /**
     * Gets the value of the currentAirlineInfo property.
     * 
     * @return
     *     possible object is
     *     {@link PaxSegmentType }
     *     
     */
    public PaxSegmentType getCurrentAirlineInfo() {
        return currentAirlineInfo;
    }

    /**
     * Sets the value of the currentAirlineInfo property.
     * 
     * @param value
     *     allowed object is
     *     {@link PaxSegmentType }
     *     
     */
    public void setCurrentAirlineInfo(PaxSegmentType value) {
        this.currentAirlineInfo = value;
    }

    /**
     * Gets the value of the flownAirlineInfo property.
     * 
     * @return
     *     possible object is
     *     {@link PaxSegmentType }
     *     
     */
    public PaxSegmentType getFlownAirlineInfo() {
        return flownAirlineInfo;
    }

    /**
     * Sets the value of the flownAirlineInfo property.
     * 
     * @param value
     *     allowed object is
     *     {@link PaxSegmentType }
     *     
     */
    public void setFlownAirlineInfo(PaxSegmentType value) {
        this.flownAirlineInfo = value;
    }

}
